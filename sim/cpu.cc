...
// Some minimal state display. If I had the time, I'd do a quick gui app, which
// is more natural
...
static void print_stages(cpu_core *core) {
  ...
  printf("(if '%s', $%-2d {$%-2d $%-2d} %08x) ",
         core->ifs.right.control()->name, core->ifs.right.Rdest,
         core->ifs.right.Rsrc1, core->ifs.right.Rsrc2,
         core->ifs.right.immediate);

  printf("(id '%s' $%-2d {$%-2d $%-2d}={%08x, %08x} %08x) ",
         core->ids.left.control()->name, core->ids.right.Rdest,
         core->ids.right.Rsrc1, core->ids.right.Rsrc2, core->ids.right.Rsrc1Val,
         core->ids.right.Rsrc2Val, core->ids.right.immediate);

  if (core->exs.busyCycles > 0) {
    printf("(ex '%s' \033[31m busy\033[0m) ", core->exs.left.control()->name);
  } else {
    printf("(ex '%s' $%-2d <= $%-2d%c($%-2d or %08x)) ",
           core->exs.left.control()->name, core->exs.right.Rdest,
           core->exs.right.Rsrc1,
           (core->exs.left.opcode == 1 || (core->exs.left.opcode == 9)
                ? '+'
                : (core->exs.left.opcode == 8 ? '-' : ' ')),
           core->exs.right.Rsrc2, core->exs.right.aluresult);
  }
  ...
  if (!core->mys.isIdle()) {
    ...
    printf(" (ms '%s'  read_mem: 0x%0X val: %08x) ",
           core->mys.left.control()->name, core->mys.left.aluresult,
  ...
  if (core->wbs.left.control()->register_write) {
    if (core->wbs.left.control()->mem_to_register) {
      printf(" (wb '%-s' $%-2d<=%08x)", core->wbs.left.control()->name,
             core->wbs.left.Rdest, core->wbs.left.mem_data);
    } else {
      printf(" (wb '%-s' $%-2d<=%08x )", core->wbs.left.control()->name,
             core->wbs.left.Rdest, core->wbs.left.aluresult);
    }
  } else {
...
void run_cpu(memory *mem, const bool verbose_cpu, const int bp_type) {
  ...
  try {
    ...
    // printf("BP miss rate: %f\n",(double)core.BPMisses/
    // (double)(core.BPHits+core.BPMisses));
  }
  catch (const char *e) {
    printf("CPU fault: %s\n", e);
  }
