...
// const uint32_t CACHE_MISS_NUMBER_OF_STALLS = 4;			// 4
// stall
// cycles for cache miss (5 cycles)
extern uint32_t cache_miss_penalty;

void CacheCtrl::cycle() {
  if (!requestQueue.empty()) {
    assert(requestQueue.size() == 1); // in this simple pipeline simulator, we
                                      // can only have one request from MEM
                                      // stage.
    ...
    if (blk && blk->isValid()) {
      ...
      if (pendingMissReq == NULL) { // only support one outstanding miss request
        ...
        pendingMissReq = pkt;
        pendingMissReq->readyCycleCount += cache_miss_penalty;

        requestQueue.pop_front();
        misses++;
      }
    }
  } else if (cache && !pendingMissReq &&
             prefetcher) { // no other miss req, let's do a prefetch
    ...
    while (!(prefetcher->prefetchQueue.empty())) {
      ...
      if (!cache->findBlock(prefetch_addr)) {
        pendingMissReq = new CachePacket(core->cycles, 0, // not used
                                         prefetch_addr, CachePrefetchReq,
