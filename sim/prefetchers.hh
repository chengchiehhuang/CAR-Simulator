...
public:
  StridePrefetcher(SimpleCache *_cache, uint32_t _size,
  ...
  bool updatePrefetchQueue(uint32_t pc, uint32_t miss_addr) {
    ...
    for (int i = 0; i < degree; i++) {
      ...
      uint32_t prefetch_addr =
          RPT[index].last_addr + RPT[index].stride * (i + 1);
  ...
  void updateTable(uint32_t pc, uint32_t miss_addr) {
    ...
    if (RPT[index].tag != pc) {
      ...
      bool incorrect =
          (miss_addr != (RPT[index].last_addr + RPT[index].stride));
      ...
      } else if (incorrect && RPT[index].state ==
                                  (Transient || RPT[index].state == NoPred)) {
      ...
      } else if (!incorrect && (RPT[index].state == Initial ||
                                RPT[index].state == Transient ||
  ...
  bool updatePrefetchQueue(uint32_t pc, uint32_t miss_addr) {
    ...
    for (int i = 0; i < 8; i++) {
      if (prefetchQueue.size() > MAX_PREFETCH_QUEUE_SIZE)
        prefetchQueue.pop_front();
      uint32_t prefetch_addr =
          ((miss_addr / cacheline_size) + (i + 1)) * cacheline_size;
      prefetchQueue.push_back(prefetch_addr);
    }
    return NULL;
  }
