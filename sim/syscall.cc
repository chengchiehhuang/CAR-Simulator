...
void sysc_op(cpu_core *cpu) {
  ...
  if (cpu->verbose)
    printf("syscall %-2d: ", cpu->registers[get_reg('v', 0)].value);
  ...
  case 4:
    // print string starting at address contained in A0 (FIXME: this does not
    // increment memory counters!)
    printf("%s", (const char *)cpu->mem->crackaddr(
                     cpu->registers[get_reg('a', 0)].value));
    fflush(stdout);
    break;
  ...
  case 8: {
    // read string, and put it into memory starting at A0 and running for up to
    // A1 bytes
    int32_t count = 0, c = 0;
    int32_t length = cpu->registers[get_reg('a', 1)].value;
    // Do things the hard way so that memory statistics are right.
    // (could just crackaddr and write directly, since virtual addresses are
    // mem-mapped into this process)
    while ((c = getchar()) && count < (length - 1)) {
      cpu->mem->set<byte>(cpu->registers[get_reg('a', 0)].value + count++, c);
      if (c == 0x0a)
        break; // on newline, break so that we mimic 'gets'
    }
    cpu->mem->set<byte>(cpu->registers[get_reg('a', 0)].value + count++,
                        0); // add on null char
  } break;
