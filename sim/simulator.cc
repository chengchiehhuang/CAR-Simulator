...
static void usage(char *name) {
  cout << name << " usage:\n"
       ...
       << "\t-o cache_size: cache size (default =0) \n"
       << "\t-c cache_assoc: 1/2/4/8 (default = 2)\n"
       << "\t-l cacheline_size: cacheline size (default = 32) \n"
       << "\t-n cache_miss_penalty: penalty for cache miss (default = 4 "
          "cycles)\n"
       << "\t-b bp_type: branch predictor type \n"
       << "\t-o enable_prefetcher: enable hardware prefetcher \n"
       << "\t-x prefetcher: prefetcher type \n" << endl;
}
...
int32_t main(int32_t argc, char **argv) {
  ...
  while ((ch = getopt(argc, argv, "t:d:vb:s:a:l:ox:n:")) != -1) {
    switch (ch) {
    case 't': {
    ...
    case 's': {
      ...
      if (size_in_kb != 0 && size_in_kb != 1 && size_in_kb != 2 &&
          size_in_kb != 4 && size_in_kb != 8) {
    ...
    case 'l':
      ...
      if (cacheline_size != 8 && cacheline_size != 16 && cacheline_size != 32 &&
          cacheline_size != 64) {
        cout << "cache line size = " << cacheline_size << " is not supported"
             << endl;
        exit(20);
      }
      break;
    ...
    case 'x':
      ...
      if (prefetcher_type != 0 && prefetcher_type != 1 &&
          prefetcher_type != 2) {
        cout << "prefetcher_type = " << prefetcher_type << " is not supported"
             << endl;
        exit(20);
      }
      break;
    case 'n':
      cache_miss_penalty = atoi(optarg);
      cout << "cache_miss_penalty = " << cache_miss_penalty << endl;
      break;
