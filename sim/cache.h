...
struct CachePacket {
  ...
  CachePacket(uint32_t curCycle, uint32_t _pc, uint32_t _addr,
              CacheRequestType _reqType, int _size, uint32_t _data = 0) {
    pc = _pc;
    addr = _addr;
    reqType = _reqType;
    size = _size;
...
public:
  CacheCtrl(cpu_core *c) {
    ...
    misses = 0;
    if (cache_size > 0) {
      printf("Cache Config: Size = %d b, Assoc = %d, Line Size = %d b, "
             "Prefetcher = %s\n",
